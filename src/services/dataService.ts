      ...
      .map((item, index) => ({
        ...
        processingDays:
          ...
          Number.isFinite(item.processingDays)
            ...
            : 3,
        processingDayType:
          item.processingDayType === "family" ? "family" : "processing",
        trainingStartDate:
        ...
        personalEndDates:
          ...
          !Array.isArray(item.personalEndDates)
            ? Object.fromEntries(
                Object.entries(item.personalEndDates).filter(
                  ([userId, dateValue]) =>
                    Boolean(userId) &&
                    typeof dateValue === "string" &&
                    /^\d{4}-\d{2}-\d{2}$/.test(dateValue)
...
const getSheetsStatusColor = (
  ...
  // הסטטוסים המובנים ממשיכים לקבל את צבעי הפסטל הקבועים בסקריפט.
  // צבע נשלח רק לסטטוס שהוגדר במפורש כצבע מותאם אישית.
  if (status?.colorKey !== "custom") return "";
