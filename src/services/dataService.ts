...
export const dataService = {
  ...
  async getAllUsers(): Promise<UserProfile[]> {
    if (!isFirebaseActive) {
      return JSON.parse(localStorage.getItem("idf_profiles") || "[]");
    }
  ...
  async createAttendanceReport(reportData: Omit<AttendanceReport, "reportId">): Promise<string> {
    const reportId = !isFirebaseActive ? `rep_${Date.now()}` : "";
    const isAlert = reportData.status !== "base";
    const statusLabel = ATTENDANCE_STATUS_LABELS[reportData.status]?.label || reportData.status;
    const notificationMsg = `החייל/ת ${reportData.userName} דיווח/ה על סטטוס ${statusLabel} מחוץ לבסיס במיקום: ${reportData.location}`;
    ...
    if (!isFirebaseActive) {
      ...
      if (isAlert) {
        ...
        const newNot: AppNotification = {
          ...
          soldierName: reportData.userName,
    ...
    const path = "attendance";
    try {
      const docRef = await addDoc(collection(db, "attendance"), reportData);
