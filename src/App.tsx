...
export default function App() {
  ...
  const [regRole, setRegRole] = useState<"soldier" | "commander">("soldier");
  ...
  // Poll reports and notifications every 4 seconds in commander mode to pop up real-time soldier reports
  useEffect(() => {
    const poll = async () => {
      if (userProfile && userProfile.role === "commander") {
        try {
          const updatedReports = await dataService.fetchAllReports();
  ...
  const handleIdLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (foundProfile) {
        ...
        if (foundProfile.role === "commander") {
          setActiveTab("dashboard");
        } else {
          setActiveTab("reporter");
        }
  ...
  const handleIdRegistrationSubmit = async (e: React.FormEvent) => {
    ...
    if (regRole === "commander" && regPasscode.trim() !== "997") {
      setLoginError("קוד אימות מפקד שגוי! לא ניתן להירשם כמפקד ללא קוד אישור גדודי תקין.");
      return;
    }
            ...
            {!isRegisteringId ? (
              ...
              >
                <div className="bg-slate-900/60 p-4 border border-slate-800 rounded-xl text-xs text-slate-300 leading-relaxed space-y-1.5 shadow-inner">
                  <span className="font-bold text-emerald-400 block mb-1 text-sm">הנחיות הזדהות גדודית:</span>
                  <p>• התחברות באמצעות **מספר אישי** (למשל: <code className="bg-emerald-950 text-emerald-300 px-1.5 py-0.5 rounded font-bold">1234567</code> לכניסה כמפקד/ת)</p>
                  <p>• או באמצעות **תעודת זהות** (למשל: למדווחים רגילים: <code className="bg-slate-950 text-slate-300 px-1.5 py-0.5 rounded font-bold">8123451</code>)</p>
                  <p>• כניסה מוגנת וקבועה – המכשיר הנוכחי יישאר מחובר אוטומטית לעמוד האישי!</p>
                </div>
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pb-1">
                  <div className="space-y-1.5">
                    <label className="block text-xs font-bold text-slate-200">מחלקה / פלוגה בגדוד</label>
                    <select
                      value={regUnit}
                    ...
                    >
                      {IDF_UNITS.map(u => (
                        <option className="bg-slate-950 text-white" key={u} value={u}>{u}</option>
                      ))}
                    </select>
                  </div>
                ...
                {regRole === "commander" && (
                  <div className="space-y-1.5 p-3.5 bg-emerald-950/20 border border-emerald-800/20 rounded-xl animate-fade-in relative">
                    <label className="block text-xs font-bold text-emerald-300">קוד אימות מפקד גדודי (לסימולציה: 997)</label>
                    <input
                      type="password"
                      required
  ...
  return (
    <div id="full-idf-app-interface" className="min-h-screen bg-military-50 flex flex-col pb-12">
      ...
      <div id="app-alerts-toaster" className="fixed top-6 left-6 z-[9999] flex flex-col gap-3 max-w-sm w-full font-sans text-right" dir="rtl">
        <AnimatePresence>
          {toasts.map((toast) => {
            return (
              <motion.div
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8 w-full flex-grow">
        ...
        {/* Navigation Tabs (Only if Commander, since Commanders are also Soldiers who can report presence) */}
        ...
        <AnimatePresence mode="wait">
          {activeTab === "reporter" || userProfile.role !== "commander" ? (
            <motion.div
              key="reporter-tab"
